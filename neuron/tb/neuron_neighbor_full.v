//2017.3.14 testbench for a neuron and a router
//          test 2 packets generated by neuron and send to router, router compute new address 
//          and forward to next router
//          there is a conflict in this testbench
//          it works, no bugs found
//          test the situation when east neighbor's fifo is full, all request of east port should be stalled.
//          it works
`timescale 1ns/100ps
`define tpd_clk 10


module neuronTB_neighbor_full;
...
.local_neuron_full(0), .north_neighbor_full(0), .south_neighbor_full(0), .east_neighbor_full(east_neighbor_full), .west_neighbor_full(0));
...
//east neighbor router clock
	always
		begin
			#5 clk_east <= ~clk_east  ;
		end

//west neighbor router clock
	always
		begin
			#5 clk_west <= ~clk_west  ;
		end

// simulate east_neighbor_full is 1 and then change to 0
    initial
        begin
            east_neighbor_full = 1;
            #400 east_neighbor_full = 0;
        end


// west port write data
initial
    begin
        //modify this delay to cause contention
        //now local port request east port first, during transmission of local port, west port request east port
        #271  write_west = 1;
            west_in = 4'h3;
        #5 west_in = 4'h4;
        #10 west_in = 4'h7;
